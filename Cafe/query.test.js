...
describe("SQL tests", function () {
  it("Basic SELECT tests", function () {
    var numbers = [1, 2, 3];
    assertSimilar(query().select().from(numbers).execute(), numbers);
    assertSimilar(
  ...
  it("Basic SELECT and WHERE over objects", function () {
    ...
    assertSimilar(query().select().from(persons).execute(), persons);
});
    ...
    assertSimilar(query().select(profession).from(persons).execute(), [
      "teacher",
      "teacher",
      "teacher",
      "scientific",
  ...
  it("join tests", function () {
    ...
    assertSimilar(query().select().from(numbers1, numbers2).execute(), [
      [1, 4],
      [1, 5],
      [2, 4],
      [2, 5],
